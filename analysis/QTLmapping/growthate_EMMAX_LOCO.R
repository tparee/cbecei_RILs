...
threshold_alpha0.05 = -log10(quantile( permutated_pvals , prob = 0.05)) # 7.099286 
threshold_alpha0.1 = -log10(quantile( permutated_pvals , prob = 0.1)) # 6.653547 
...
pgwas = ggplot(res_LOCO)+
  ...
  coord_cartesian(ylim = c(0,-log10(min(res_LOCO$pval))+0.6))+
  ...
  geom_point(alpha=1, size=0.25,  aes(cm, -log10(pval)))+
