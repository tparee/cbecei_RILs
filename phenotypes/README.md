...
- `hours_to_starve`  
Number of hours until resource exhaustion. Measurements were obtained by placing two adult females and two adult males on a plate seeded with *E. coli* and monitoring the population until it peaked and subsequently crashed due to starvation. Plates were imaged hourly to estimate the starvation time (see https://doi.org/10.1093/genetics/iyaf073).
...
Contains raw worm length measurements. Worms were imaged 48 hours after the L1 stage. Imaging was performed in 96-well plates using an ImageXpress instrument. Worm objects were detected and measured using the Andersen lab pipeline (see https://doi.org/10.1371/journal.pone.0252000).
...
- Mean worm length (`µm`) for females and males, unscaled (`mean_worm_length_um_f`, `mean_worm_length_um_m`) and 
scaled (variance = 1 and mean = 0 within each sex; `scaled_mean_worm_length_um_f`, `scaled_mean_worm_length_um_m`):
 
- Standard errors: `(scaled_)SE_f` & `(scaled_)SE_m`
