...
InferFoundersHaploBlocksWIN = function(win,foundersgeno_ll, rilsA, rilsB, info){
  
  # List crontaining rils genotype within the window for cross A and cross B
  rilgeno = list(A=rilsA[win,], B=rilsB[win,])
  
  fgt_lls = foundersgeno_ll[win,]
  best_ll = sum(apply(fgt_lls,1,max)) # the ll if all the most likely individual genotypes 
  ...
  rilshap = lapply(rilgeno, function(gx) {
    ...
    npergroup = npergroup[npergroup > 5] # Keep only clusters with more than 5 RILs (I don't have a good rational for this number, could be more)
 ...
 # rilshap is a list of two genotype matrix [snp,haplo] corresponding to cross A or B
  ...
  nperhap = c(as.numeric(colnames(rilshap[["A"]])[-sharedRilsHaplo$hA]),
              as.numeric(colnames(rilshap[["B"]])[-sharedRilsHaplo$hB]),
              as.numeric(colnames(rilshap[["A"]])[sharedRilsHaplo$hA]) + as.numeric(colnames(rilshap[["B"]])[sharedRilsHaplo$hB]))
  ...
  similarRilsHaplo = setNames(
    as.data.frame(t(apply(t(combn(1:ncol(rilshap), m = 2)), 1, function(pair){
    hA = rilshap[,pair[1]]
    hB = rilshap[,pair[2]]
    nmissmatch = sum(hA!=hB, na.rm=T)
   return(c(pair,nmissmatch))
  }))), c("h1", "h2", "nmissmatch")
  )
  similarRilsHaplo = similarRilsHaplo[similarRilsHaplo$nmissmatch==1,]
  
  
  founders_possible_diplotypes = lapply(c("A", "B", "M"), function(thisfounder){
  ...
  names(founders_possible_diplotypes)=c("A", "B", "M")
  
  founders_combinations = as.matrix(do.call(expand.grid, lapply(founders_possible_diplotypes, function(x){1:nrow(x)})))
  founders_combinations = as.data.frame(founders_combinations)
  
  # remove combination where not all RILs haplotypes are represented
  # Allow scenario where one haplotype is not represented if it only differ by one single snp with a represented haplotype
  founders_combinations = cbind(founders_combinations, nmissrilhap = unlist(lapply(1:nrow(founders_combinations), function(i){
    
    fcomb = unlist(founders_combinations[i,])
    representedrilshaplo = unique(unlist(lapply(names(fcomb), function(thisfounder){
      unlist(founders_possible_diplotypes[[thisfounder]][fcomb[thisfounder],])
    })))
    
    representedrilshaplo = 1:ncol(rilshap) %in% representedrilshaplo
    
    for(nr in which(!representedrilshaplo)){
      similarone_is_represented = F
      if(nr %in% similarRilsHaplo$h1 | nr %in% similarRilsHaplo$h2){
        similarone_is_represented = sum(similarRilsHaplo[which(similarRilsHaplo$h1 == nr | similarRilsHaplo$h2 == nr),c("h1","h2")] %in% which(representedrilshaplo)) > 0
        if(similarone_is_represented){representedrilshaplo[nr]=T}
      }
    }
    
    sum(nperhap[!representedrilshaplo])
    
    #pass = ifelse(sum(!representedrilshaplo)>0, F,T)
    #if(pass){return(founders_combinations[i,])}else{return(NULL)}
  })))
  
  founders_combinations = founders_combinations[founders_combinations$nmissrilhap < 40,]
  
  
  founders_combinations$ll =unlist(lapply(1:nrow(founders_combinations), function(i){

    fcomb = unlist(founders_combinations[i,1:3])
    
    founders_possible_diplotypes
    fgeno_codes = do.call(cbind,lapply(names(fcomb), function(thisfounder){
      hapcoord = unlist(founders_possible_diplotypes[[thisfounder]][fcomb[thisfounder],])
      fgt = apply(rilshap[,hapcoord],1,mean)
      fgt = round(fgt, digits=1)
      return(paste0("F",thisfounder,":", as.character(fgt)))
      }))
    
    fgeno_codes  = apply(fgeno_codes,1,paste, collapse=";")
    #cbind(1:nrow(rilshap), match(fgeno_codes,colnames(foundersgeno_ll)))
    ll = sum(fgt_lls[cbind(1:nrow(rilshap), match(fgeno_codes,colnames(fgt_lls)))])
    ll
  }))
  
  mostlikely = which.max(founders_combinations$ll)
  max_ll = founders_combinations$ll[mostlikely]
  
  fcomb = unlist(founders_combinations[mostlikely,])
  fhap = do.call(c,lapply(names(fcomb)[1:3], function(thisfounder){
    hapcoord = unlist(founders_possible_diplotypes[[thisfounder]][fcomb[thisfounder],])
    hapcoord
  }))
  ...
  rilgeno=do.call(cbind, rilgeno)
  rilgeno[rilgeno==0.5]=NA
  nbreaks=countBreaks(windows=c(1,length(win)), rilgeno, fhap, info[win,])$nbreak
  
  #gx = cbind(fhap, rilgeno)
  ...
  return(list(win=win,foundergt=fhap, heterozygous = het, rilshap=whichrilshap, nbreaks = nbreaks, max_ll = max_ll, best_ll = best_ll, allRepresented=allRepresented, gsize=diff(range(info$cM[win]))))
}


get_removed_snps = function(founderhaplotypes){
  unlist(lapply(founderhaplotypes, function(x){
    full = min(x$win):max(x$win)
    full[!(full %in% x$win)]
  }))
}

removesnp = function(founderhaplotypes, snps_to_remove){
  lapply(founderhaplotypes, function(x){
    rsnps = x$win %in% snps_to_remove
    if(sum(rsnps)>0){
      x$win = x$win[!rsnps]
      x$foundergt = x$foundergt[!rsnps,]
      x$heterozygous = c(sum(x$foundergt[,1]!=x$foundergt[,2]), sum(x$foundergt[,3]!=x$foundergt[,4]), sum(x$foundergt[,5]!=x$foundergt[,6])) > 0
      }
    return(x)
  })
}
...
check_adjacent_compatibility = function(founderhaplotypes){
  lapply(1:(length(founderhaplotypes)-1), function(i){ 
    #print(i)
    ...
    #paste((which(ovfi != ovfj, arr.ind = TRUE)[,1]), collapse=";")
    ...
    if(sum(!iscompatible) > 0){
      incomp_snps = unique(which(ovfi != ovfj, arr.ind = TRUE)[,1])
      incomp_snps = winoverlap[incomp_snps]
      n_incomp_snps = length(incomp_snps)
    }else{incomp_snps="NONE"; n_incomp_snps=0}
    
    list(win=i, incomp_snps=incomp_snps,  n_incomp_snps= n_incomp_snps)
...
resolve_incompatibility_by_merging_win = function(I, founderhaplotypes, pushmax = 4, pushdirection = 1){
  
  # I = the nth elemement of  founderhaplotypes (e.g, I = 3)
  # I can be a vector of adjacent problematic window (e.g, I = c(3,4))
  push = 1
  iscompatible = F
  
  while(!iscompatible & push <= pushmax){
    print(push)
    
    if(pushdirection == 1){
      whichwindows = min(I):(max(I)+push) # target the window within the incompatibility groups and the next window as well
    }
    
    if(pushdirection == -1){
      whichwindows = (min(I)-push):max(I) # target the window within the incompatibility groups and the next window as well
    }
    
    if(pushdirection == "both"){
      whichwindows = (min(I)-push):(max(I)+push) # target the window within the incompatibility groups and the next window as well
    }
    ...
    newfhap = InferFoundersHaploBlocksWIN(win=winall,
                                          foundersgeno_ll=foundersgeno_ll,
  ...
  if(!iscompatible){ return(F)}
  newfhap = c(newfhap, list(whichwindows=whichwindows))
  return(newfhap)
}

InferFoundersHaploBlocks = function(foundersgeno_ll, rilsA, rilsB, info,nsnpWin = 500, maxSizeCM=3){
  ...
  print("Inferring founder haplotype for window:")
  ...
  # If it is not the case, it means that the two different window result in inferred haplotypes that cannot be the same
  # for each  window i
  print("Searching for incompatibilities between adjacent windows")
  compatibletest = check_adjacent_compatibility(founderhaplotypes = founderhaplotypes)
  
  #identify the incompatible windows
  # ex: incompatibility = 10 => means that window 10 and 11 and incompatible 
  incompatibility_1snp = which(unlist(lapply(compatibletest,function(x){x$n_incomp_snps == 1})))
  incompatibility_multiplesnp = which(unlist(lapply(compatibletest,function(x){x$n_incomp_snps > 1})))
  
  if(length(incompatibility_1snp)>0){
    problematic_snps = unlist(lapply(compatibletest[incompatibility_1snp], function(x){x$incomp_snps}))
    founderhaplotypes = removesnp(founderhaplotypes, snps_to_remove=problematic_snps)
  }
  
  
  incompatibility = incompatibility_multiplesnp
  if( length(incompatibility)>0 ){ # if there are some incompatible window
    ...
    # incompatiblility = 9 11 12 40  => incompatiblility = list(9, c(11,12), 40)
    ...
    compatibleHaplo = list(NULL)
    while(length(incompatibility) > 0){
      print(incompatibility[[1]])
      merged = resolve_incompatibility_by_merging_win(I=incompatibility[[1]], founderhaplotypes)
      if(!is.list(merged)){merged = resolve_incompatibility_by_merging_win(I=incompatibility[[1]], founderhaplotypes, pushdirection = "both", pushmax = 1)}
      if(!is.list(merged)){stop("code needs to be improved here")}
      compatibleHaplo = c(compatibleHaplo, list(merged))
      incompatibility = lapply(incompatibility, function(x){x[!(x %in% merged$whichwindows)]})
      incompatibility = incompatibility[lapply(incompatibility,length) > 0]
    }
    compatibleHaplo = compatibleHaplo[-1]
    
    incompatibility = lapply(compatibleHaplo, function(x){x$whichwindows})
    compatibleHaplo = lapply(compatibleHaplo, function(x){x[-which(names(x)=="whichwindows")]})
    
    # Just replace the problematic windows with the extended windows
    for(i in 1:length(incompatibility)){
      founderhaplotypes[[ min(incompatibility[[i]]) ]] <- compatibleHaplo[[i]]
    }

    founderhaplotypes = founderhaplotypes[-unlist(sapply(incompatibility, function(x){x[-1]}))]
...
phaseHaplotypes = function(founderhaplotypes, info){
  ...
  # This phasing occurs in two main step
  ...
  phasedfounderhaplotypes=lapply(1:3, function(thisfounder){
    ...
    wblockx = 1:(max(phap$block)-1)
    wblockx = wblockx[!duplicated(wblockx) & wblockx > 0]
    
    blockphase=do.call(rbind, lapply(wblockx, function(i){
      print(i)
      j=i+1
      ...
      # If genetic size too long (>5 cM), we are going to trim a bit the blocks
